...
from .forms import CustomUserCreationForm, NewsLetter
from .models import *
...
def home(request):
    if request.user.is_authenticated:
        ...
        cartItems = ""
        wishlistItems = ""
    rate = requests.get("https://v6.exchangerate-api.com/v6/081e9af75e1a2610c8a65065/latest/INR").json()
    Rate = ''
    currency = ''
...
def ship_and_return(request):
    return render(request, 'shipping_and_return.html')
...
def newsletter(request):
    form = NewsLetter(request.POST.get('email'))
    if request.method == 'POST':
        form.save()
